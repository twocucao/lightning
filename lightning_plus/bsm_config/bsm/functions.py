...
from lightning_plus.bsm_config.models import Setting, FieldPermission, Admin, FieldAdmin
...
def get_assign_permissions(user, **kwargs):
    permissions = get_objects_for_user(user, "auth.permission_assign").select_related(
        "content_type"
    )
...
def update_field_permission(user, model, permissions, **kwargs):
    ...
    with transaction.atomic():
        for permission in permissions:
            ...
            field_admin, field_admin_created = FieldAdmin.objects.get_or_create(
                admin=admin, field=field
            )
            ...
            (
                ...
                field_admin=field_admin, group_id=group
            )
...
def get_field_permissions(user, model, **kwargs):
    field_admins = FieldAdmin.objects.filter(admin__model=model).prefetch_related(
        "fieldpermission_set"
    )
    next_permissions = []
