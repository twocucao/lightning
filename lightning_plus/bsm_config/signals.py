...
from lightning_plus.bsm_config.models import Menu, Admin, Setting
...
def remove_menu_permission(sender, instance, update_fields=[], **kwargs):
    if instance._state.adding:
        """把新建的菜单排序到最前面"""
        min_sequence = (
            Menu.objects.exclude(sequence=0)
            .values_list("sequence", flat=True)
            .annotate(Min("sequence"))
            .order_by("sequence")
            .first()
            or 1000
        )
...
def update_setting_config_permission(sender, **kwargs):
    ...
    Permission.objects.get_or_create(
        content_type=permission_assign_content_type,
        codename="permission_assign",
        name="权限分配",
    )
...
def get_actions(config):
    result = []
    for action in (
        config.get("inlineActions", [])
        + config.get("actions", [])
        + config.get("tableActions", [])
    ):
        if "id" not in action:
...
def update_action_permission(app, model, new_config, old_config):
    ...
    create = [
        ...
        if action["id"] in list(new_action_ids - old_action_ids)
    ]
    update = [
        action
        for action in new_actions
    ...
    delete = [
        ...
        and action["id"] in list(old_action_ids - new_action_ids)
    ]
...
def delete_action_permission(app, model, config):
    if get_actions(config):
        content_type = ContentType.objects.get(app_label=app, model=model)
        actions = [
            c for c in get_actions(config) if isinstance(c, dict) and "groups" in c
        ]
        filtered_actions = [
            f'{action["action"]}_{model}_{action["id"]}' for action in actions
        ]
        Permission.objects.filter(
            code_name__in=filtered_actions, content_type=content_type
        ).delete()
...
def admin_change(sender, instance, create, request, old_instance, **kwargs):
    ...
    if create:
        ...
        update_action_permission(
            *instance.model.split("__"), instance.config, old_instance.config
        )
