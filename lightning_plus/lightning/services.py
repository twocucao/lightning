    ...
    for menu_data in cust_menus:
        ...
        if children:
            for child_menu in children:
                menus_mapping.update(
                    {child_menu.get("name", None): menu_data.get("name", None)}
                )
...
def create_admin_config(app_labels, exist_model):
    ...
    models = [
        ...
        and f"{model._meta.app_label}__{model._meta.model_name}" not in exist_model
    ]
    ...
    for model in models:
        ...
        config = {
            "display": [],
            "formFields": [],
            "inlineActions": ["edit", "delete", "detail"],
            "filter": [],
        }
        ...
        for field in model._meta.fields:
            ...
            if type(field) not in [AutoField, BigAutoField] or not (
                field.primary_key and type(field) in [AutoField, BigAutoField]
            ):
        ...
        if detailTableFields:
            detail["sections"].append(
                {"fields": detailTableFields, "style": {"widget": "tiles"}}
            )
        if detailFields:
    ...
    Menu.objects.bulk_create(
        [menu for menu in menus if f"{menu.model}-{menu.page}" not in menu_keys]
    )
...
@transaction.atomic
def generate_configs(app_labels=None):
    ...
    for app in app_labels:
        ...
        print("-------->")
        old_models = [
            admin.model for admin in admins if admin.model.startswith(f"{app}__")
        ]
        print(old_models)
