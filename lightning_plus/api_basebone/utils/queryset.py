...
from ..restful.serializers import (
    ...
    get_field,
    nested_display_fields,
    sort_expand_fields,
    display_fields_to_expand_fields,
)
...
def annotate_queryset(queryset, fields=None, context=None):
    ...
    if "GMeta" in queryset.model.__dict__:
        ...
        annotated_fields = getattr(
            queryset.model.__dict__["GMeta"], gmeta.GMETA_ANNOTATED_FIELDS, {}
        )
        ...
        for name, field in annotated_fields.items():
            if "annotation" in field:
                params[name] = (
                    ...
                    else field["annotation"](context or {})
                )
...
def expand_dict_to_prefetch(
    model, expand_dict=None, fields=None, context=None, display_fields=None
):
    result = []
    if expand_dict is None:
        if display_fields is not None:
            expand_dict = sort_expand_fields(
                display_fields_to_expand_fields(display_fields)
            )
        else:
            expand_dict = {}
    ...
    for key, value in expand_dict.items():
        ...
        pfs = expand_dict_to_prefetch(
            next_model,
            value,
            fields=next_fields,
            context=context,
            display_fields=nested,
        )
        # if not pfs:
        # 是否能节省资源？
        #     result.append(key)
        #     continue
        qs = next_model.objects.defer(
            *get_exclude_fields_by_model(next_model)
        ).prefetch_related(*pfs)
        if display_fields is not None and nested:
            qs = queryset_only(qs, nested)
        # 使关联关系也能用annotated_field
        prefetch = Prefetch(
            key, queryset=annotate_queryset(qs, fields=next_fields, context=context)
        )
...
def queryset_only(queryset, display_fields):
    annotated_fields = get_attr_in_gmeta_class(
        queryset.model, gmeta.GMETA_ANNOTATED_FIELDS, {}
    )
    computed_fields = get_attr_in_gmeta_class(
        queryset.model, gmeta.GMETA_COMPUTED_FIELDS, []
    )
    computed_field_names = {c["name"] for c in computed_fields}
    only = [
        d
        for d in display_fields
        if "." not in d and d not in annotated_fields and d not in computed_field_names
    ]
    for d in display_fields:
        if "." in d:
...
def queryset_prefetch(queryset, expand_dict=None, context=None, display_fields=None):
    ...
    return queryset.defer(
        *get_exclude_fields_by_model(queryset.model)
    ).prefetch_related(
