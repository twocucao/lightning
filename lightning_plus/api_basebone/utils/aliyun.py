...
class AliYunOSS:
    ...
    def auth(self):
        if self._auth is None:
            self._auth = oss2.Auth(
                site_setting[self.OSS_KEY], site_setting[self.OSS_SECRET]
            )
        return self._auth
    ...
    def bucket(self):
        if self._bucket is None:
            self._bucket = oss2.Bucket(
                self.auth,
                site_setting[self.OSS_ENDPOINT],
                site_setting[self.OSS_BUCKET],
            )
        return self._bucket
    ...
    def upload_file(self, remote_file, local_file, replace=True):
        ...
        try:
            ...
            if not exists:
                status = self.bucket.put_object_from_file(
