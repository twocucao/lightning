...
def cmp_wrap(func):
    return lambda *args: bool(
        reduce(lambda a, b: a is not False and func(a, b) and b, args)
    )
...
def resolve_expression(expression, variables=None):
    ...
    if matched:
        ...
        if func == "__variables__":
            return variables
        args = [
            resolve_expression(buffer, variables=variables)
            for buffer in split_expression(arg_str, ",")
        ]
