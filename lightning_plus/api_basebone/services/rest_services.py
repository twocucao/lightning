...
from lightning_plus.api_basebone.restful.funcs import find_func
from lightning_plus.api_basebone.restful.relations import (
    forward_relation_hand,
    reverse_relation_hand,
)
from lightning_plus.api_basebone.drf.response import success_response

from lightning_plus.api_basebone.restful.client import user_pip as client_user_pip
    ...
    for k, v in record.items():
        ...
        if isinstance(v, list):
            ...
            if (star_key not in display_fields_set) and (
                ...
                """星号为通配符"""
                continue
            display_record[k] = filter_sub_display_fields(
                display_fields_set, v, full_key
            )
        # 星号优先级高于具体的列名
...
def manage_create(genericAPIView, request, set_data):
    ...
    with transaction.atomic():
        forward_relation_hand(genericAPIView.model, set_data)
        serializer = genericAPIView.get_validate_form(genericAPIView.action)(
            data=set_data, context=genericAPIView.get_serializer_context(), many=many
        )
        serializer.is_valid(raise_exception=True)
        instance = genericAPIView.perform_create(serializer)
