...
from lightning_plus.api_basebone.core.fields import JSONField, ArrayField
from lightning_plus.api_basebone.export.fields import DJANGO_FIELD_TYPE_MAP
...
class BSMField:
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if (
            cls not in ModelSerializer.serializer_field_mapping
            or cls.__name__ not in DJANGO_FIELD_TYPE_MAP
        ):

            class BSMFieldSerializer(instance.super_serializer()):
                def to_internal_value(self, value):
                    return instance.bsm_to_internal_value(
                        super().to_internal_value(value)
                    )

                def to_representation(self, value):
                    return super().to_representation(
                        instance.bsm_to_representation(value)
                    )

            ModelSerializer.serializer_field_mapping[cls] = BSMFieldSerializer
            DJANGO_FIELD_TYPE_MAP[cls.__name__] = DJANGO_FIELD_TYPE_MAP[
                instance.get_to_field()
            ]
        return instance
...
def define_bsm_field(
    base, to_type=None, to_internal_value=None, to_representation=None
):
    if to_type is None:
        to_type = base()
    ...
    ModelSerializer.serializer_field_mapping[field_cls] = type(
        field_cls.__name__,
        (
            FieldSerializerMixin,
    ...
    DJANGO_FIELD_TYPE_MAP[field_cls.__name__] = DJANGO_FIELD_TYPE_MAP[
        to_type.get_bsm_internal_type()
        if hasattr(to_type, "get_bsm_internal_type")
        else to_type.get_internal_type()
    ]
...
class MultiReferField(BSMField, ArrayField):
    ...
    def bsm_to_internal_value(self, value):
        return [
            item if isinstance(item, str) else item[self.to_field] for item in value
        ]
