        ...
        if model._meta.many_to_many:
            ...
            for field in model._meta.many_to_many:
                if field_info.forward_relations[field.name].has_through_model:
                    ...
                    kwargs[field.name] = serializers.PrimaryKeyRelatedField(
                        many=True, queryset=related_model.objects.all()
                    )
...
def get_form_class(
    model, action, exclude_fields=None, end=MANAGE_END_SLUG, request=None, **kwargs
):
    """获取表单类
