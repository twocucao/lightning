...
from lightning_plus.api_basebone.core.admin import VALID_MANAGE_ATTRS, BSM_BATCH_ACTION
from lightning_plus.api_basebone.restful.batch_actions import get_model_batch_actions
from lightning_plus.api_basebone.core.admin import BSMAdminModule
from lightning_plus.api_basebone.utils import meta
from lightning_plus.api_basebone.utils.format import underline_to_camel
from lightning_plus.bsm_config.models import Admin
from lightning_plus.bsm_config.bsm.functions import get_field_permissions
...
def get_app_admin_config():
    ...
    for admin in Admin.objects.all():
        ...
        config[admin.model]["field_permissions"] = get_field_permissions(
            None, admin.model
        )
...
def get_json_field_admin_config(
    json_object_schemas: dict, json_array_item_schemas: dict
):
    """
    ...
    for key, value in json_object_schemas.items():
        admin_configs[key] = {
            "formFields": [field["name"] for field in value["fields"]]
        }

    for key, value in json_array_item_schemas.items():
        admin_configs[key] = {
            "inlineFormFields": [field["name"] for field in value["fields"]]
        }
